...
st.markdown("""
**「証拠」をAIに分析させましょう。**
文章だけでなく、**「学校との録音データ」「手書きメモの写真」「時系列のExcel表」**などをアップロードすると、法律違反がないか徹底的にチェックします。
""")

# サイドバー設定
with st.sidebar:
    st.header("📂 証拠資料のアップロード")
    st.info("ここにアップロードしたデータは、AIによる分析のみに使用され、外部に保存されることはありません。")
    
    # ファイルアップローダー（複数種類対応）
    uploaded_files = st.file_uploader(
        "画像・音声・Excelを選択してください", 
        type=['png', 'jpg', 'jpeg', 'mp3', 'wav', 'm4a', 'xlsx', 'csv'], 
        accept_multiple_files=True
    )

    st.markdown("---")
    st.header("ℹ️ 利用上の注意")
    st.warning("個人情報（実名等）には十分注意してください。")
...
ユーザーは文章だけでなく、**「録音音声」「手書きメモ」「時系列表」**などの証拠資料を提示する場合があります。
これらを統合的に分析し、学校側の対応に違法性がないかチェックしてください。

【あなたの役割】
1. **証拠の解析**: 提示された音声や画像の内容を読み取る。
...
### 1. 証拠資料の確認
（アップロードされた音声や画像から、AIが読み取った内容を要約する）
「音声データからは、教頭が『〇〇』と発言していることが確認できました」など。
...
**解説:**
証拠資料（音声/メモ）では学校側はこう言っていますが、これは資料の P.〇〇 に反しています。
...
user_input = st.text_area("補足情報や相談内容を入力してください", height=150, 
    placeholder="例：アップロードした音声は昨日の校長との会話です。「調査しない」と言われた点が問題だと思うのですが、どうでしょうか？")

if st.button("証拠資料を含めて分析する", type="primary"):
    with st.spinner("証拠資料（音声・画像・テキスト）を解析中..."):
        try:
            # AIに渡すデータのリスト作成
            content_parts = []
            ...
            if not content_parts:
                ...
                if response.text:
                    ...
                    st.markdown("---")
                    st.success("AIは画像や音声の内容も理解しています。指摘された内容をメモして活用してください。")
                else:
                    st.error("分析できませんでした。")

        except Exception as e:
            st.error(f"エラーが発生しました: {e}")
            st.info("※音声ファイルが大きすぎる場合や、形式が対応していない可能性があります。")